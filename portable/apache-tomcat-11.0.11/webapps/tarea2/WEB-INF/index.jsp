...
<body>
    ...
    <div class="content">
        <div class="contenedores">
            ...
            %>
            <a href="<%=request.getContextPath()%>/consultaEvento?evento=<%=java.net.URLEncoder.encode(evento.getNombre(), "UTF-8")%>"
               class="contenedor-link">
                <div class="contenedor">
                    ...
                    <div class="informacion">
                        ...
                        <form id="form-<%=evento.getNombre().replaceAll("\\s+", "_")%>"
                              action="<%=request.getContextPath()%>/home" method="post"
                              style="display: inline;">
                            <input type="hidden" name="eventoDarBaja" value="<%=evento.getNombre()%>" />
                            <div class="boton-div">
                                <button type="button" class="btn-nuevaedicion"
                                    onclick="abrirModalBaja('<%=evento.getNombre().replace("'", "\\'")%>', event)">
                                    Dar de baja
                                </button>
                            </div>
                        </form>
